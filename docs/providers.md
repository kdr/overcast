...
- **`phone`** — reverse phone / **number OSINT** via Apify PhoneInfoga (actor override `OVERCAST_PHONE_ACTOR`, default `datacach~phoneinfoga-phone-number-osint-scanner`). Ref: `phone:<E.164>` (e.g. `phone:+14155551212`). One hit — `payload` carries the offline libphonenumber parse (`carrier` guess, `country`, `country_code`, `valid`, local/international formats) and the grouped web `footprint` (Google-dork URLs by category). `APIFY_TOKEN` only, no other key. No natural media (metadata evidence). `--limit`/`--since` ignored.
