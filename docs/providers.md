...
- [`examples/providers/sources/{youtube,tiktok,x,web,lens,dl,gdelttv,wayback,overpass,firms,flights,instagram,telegram,webcam,facesearch,yandeximg,dork,shodan,browser,username,person,phone,property,plate}.sh`](../examples/providers/sources/) — yt-dlp (youtube/dl) + Apify (tiktok/x/lens/instagram/telegram/facesearch/yandeximg + the identity sources username/person/phone/property/plate) + web-search (Tavily/Brave) + Google dorking (Serper.dev) + Shodan host recon + Google Lens & Yandex reverse-image + GDELT TV broadcast-news + Wayback Machine archive recovery + Overpass OSM features + NASA FIRMS active-fire hotspots + OpenSky ADS-B flights + Windy Webcams + headless-Chromium page render (`browser`, delegates to the screenshot engine) source providers.
