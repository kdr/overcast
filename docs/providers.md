...
- [`examples/providers/sources/{youtube,tiktok,x,web,lens,dl,gdelttv,wayback,instagram,telegram,webcam,facesearch,dork,shodan,browser,username,person,phone,property,plate}.sh`](../examples/providers/sources/) — yt-dlp (youtube/dl) + Apify (tiktok/x/lens/instagram/telegram/facesearch + the identity sources username/person/phone/property/plate) + web-search (Tavily/Brave) + Google dorking (Serper.dev) + Shodan host recon + Google Lens reverse-image + GDELT TV broadcast-news + Wayback Machine archive recovery + Windy Webcams + headless-Chromium page render (`browser`, delegates to the screenshot engine) source providers.
