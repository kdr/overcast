...
- `OVERCAST_QMD_TIMEOUT_MS` — per-step qmd rebuild timeout override; the default is derived from corpus size (15 min floor + ~10 s per doc, so a CPU-only embed of a few hundred docs doesn't get killed mid-build)
- `OVERCAST_INDEX_BATCH_WAIT_S` / `OVERCAST_INDEX_BATCH_POLL_MS` — `index add --all` wave backpressure: max wait between submission waves (default 300 s) and its poll interval (default 10 s); wave size is `--batch` (default 12, `0` = single unpaced pass)
...
- `ETHERSCAN_API_KEY` — the `chain:eth:` blockchain source (free Etherscan key); `chain:btc:` (mempool.space) is keyless
- `gdelttv`, `overpass`, `dispatch`, `browser`, `chain:btc:`, and `edgar` need **no key** (`dispatch` optionally takes a `SOCRATA_APP_TOKEN` to raise rate limits; `edgar` sends a descriptive contact-email User-Agent per SEC policy — `OVERCAST_HTTP_UA` overrides)
