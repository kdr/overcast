...
export function buildSystemPrompt(): string {
  ...
  return [
    ...
    "downloads, HTML/markdown exports), create them INSIDE this directory. Prefer its absolute path",
    "joined with a subfolder (that is correct no matter where the shell's cwd is); the base shell tools",
    "usually already start here, so a plain relative path like `./axon-videos/` normally lands here too",
    "— run `pwd` first if you are unsure, and never guess. Do NOT `cd` to a parent directory or invent a",
    "sibling path from the case name: the case folder often shares a name stem with files/dirs inside it",
    "(a case at `/data/acme` with a request for an `acme-clips` folder means `/data/acme/acme-clips`,",
    "never `/data/acme-clips`). When the user says \"the current folder\"/\"this directory\"/\"here\", they",
    "mean this case directory.",
    "",
