...
const ENV_GROUPS: Array<{ title: string; vars: Array<[string, string]> }> = [
  ...
  },
  {
    ...
    vars: [
      ...
      ["APIFY_TOKEN", "Apify token for the `tiktok`/`x`/`instagram`/`telegram`/`lens`/`facesearch` sources AND the identity sources `username`/`person`/`phone`/`property`/`plate` (all opt-in, authorized use only)"],
