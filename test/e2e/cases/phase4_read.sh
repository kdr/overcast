...
if [ -f "$real_media" ]; then
  cond "note records a real media artifact for later local case search"
  ...
  capture_cmd "overcast note 'Real media artifact: Hacker News browsing video is available for case search' --ref '$real_media' --tag real-media,artifact --json --case '$casedir'" "$real_note"
...
cond "note anchors human observations to a prior media record"
...
capture_cmd "overcast note 'Human observation: the white van has a missing rear plate' --ref '$watch_id' --at 13-16 --tag vehicle,plate --confidence high --json --case '$casedir'" "$note"
...
cond "default ask searches local-grep case memory and cites media spans"
ask="$($OVERCAST ask 'white van at the docks' --json --case "$casedir" 2>/dev/null)"
capture_cmd "overcast ask 'white van at the docks' --json --case '$casedir'" "$ask"
...
cond "default ask can retrieve human-authored notes"
note_ask="$($OVERCAST ask 'missing rear plate' --json --case "$casedir" 2>/dev/null)"
capture_cmd "overcast ask 'missing rear plate' --json --case '$casedir'" "$note_ask"
...
capture_cmd "overcast case memory index status --json --case '$casedir'" "$idx"
...
cond "qmd memory index rebuild records backend/model compatibility"
qmd_rebuild="$($OVERCAST case memory index rebuild --memory qmd --json --home "$casedir/.ochome" --case "$casedir" 2>/dev/null)"
capture_cmd "overcast case memory index rebuild --memory qmd --json --home '$casedir/.ochome' --case '$casedir'" "$qmd_rebuild"
