...
media_summary() {
  for v in OC_VIDEO_VISUAL OC_VIDEO_OBJECTS OC_VIDEO_SMALL OC_VIDEO_SPEECH OC_IMAGE OC_AUDIO OC_TEST_MEDIA_INDEX OC_TEST_FACE_INDEX OC_LOCAL_FACE_VIDEO OC_LOCAL_FACE_IMAGE OC_LOCAL_IMAGE_REF OC_LOCAL_IMAGE_VIDEO_A OC_LOCAL_IMAGE_VIDEO_B OC_LOCAL_IMAGE_FPS OC_LOCAL_FACE_FPS; do
    [ -n "${!v:-}" ] && printf '%s ' "${v#OC_}"
  done
}
