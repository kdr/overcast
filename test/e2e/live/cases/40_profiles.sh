...
show="$(ocrun "$CASE" --profile recon setup show --json 2>/dev/null)"
...
plist="$(ocrun "$CASE" --profile recon provider list --json 2>/dev/null)"
assert_nonempty "$C.provider_list" "$(echo "$plist"|jq -r '.payload.providers.see // empty | tostring')" "provider list shows see"
desc="$(ocrun "$CASE" --profile recon provider describe see --json 2>/dev/null)"
...
def="$(ocrun "$CASE" --profile default setup show --json 2>/dev/null)"
...
if [ -f "$FRAME" ]; then
  he="$(ocrun "$CASE" --profile httptest see "$FRAME" --json 2>/dev/null)"
